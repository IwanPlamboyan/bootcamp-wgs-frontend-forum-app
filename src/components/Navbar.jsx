...
const Navbar = () => {
  ...
  const { username, email, image_url } = useSelector((state) => state.auth);
    ...
    <header>
      <nav className="fixed top-0 w-full py-3 px-6 shadow-lg z-40 bg-white backdrop-blur-3xl opacity-[0.92]">
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center">
            <BiMenu className={`w-7 h-7 cursor-pointer -translate-x-7 ${username ? '' : 'hidden'}`} onClick={() => setShowSidebar(!showSidebar)} />
          ...
          <div className={`${username === false ? '' : 'hidden'}`}>
          ...
          <div className={`flex items-center gap-3 cursor-pointer z-10 ${username ? '' : 'hidden'}`} onClick={() => setOpenMenuProfile(!openMenuProfile)}>
      ...
      <div className={`fixed -top-3 z-50 right-0 bottom-0 left-0 ${openMenuProfile ? '' : 'hidden'}`} onClick={(e) => closeMenuProfile(e)}>
        <div className="absolute right-14 top-[73px] z-30 w-44 origin-top-right rounded-md bg-gray-100 shadow-md shadow-gray-400 border border-gray-400">
          ...
          <button className="w-full text-gray-600 hover:text-slate-900 hover:bg-gray-300 text-left px-6 py-2.5 text-sm" onClick={() => dispatch(logout())}>
            Logout
          </button>
        </div>
      </div>
